...
async function saveCommunityData(data) {
  await fs.writeFile(communityFile, JSON.stringify(data, null, 2))
}
...
function getImpactForUser(user) {
  ...
  return {
    ...
    monthlyTarget: 500,
    co2Saved: Number((completedCount * 1.2).toFixed(1)),
    wasteReduced: Number((completedCount * 0.8).toFixed(1)),
    waterSaved: completedCount * 15,
    greenDistance: Number((completedCount * 2.5).toFixed(1)),
    treesEquivalent: Number((completedCount / 4).toFixed(1)),
  ...
  res.json({
    ...
    stats: {
      ...
      actionsToday: usersData.users.reduce(
        (total, user) => total + (user.activities || []).length,
        0
      ),
      co2Saved: Number((
        usersData.users.reduce(
          (total, user) => total + (user.completedMissionIds || []).length,
          0
        ) * 1.2
      ).toFixed(1))
    },
...
app.post('/api/community/posts', async (req, res) => {
  ...
  const category = String(req.body.category || 'Eco Action').trim()
  const usersData = await getUsersData()
  const user = usersData.users.find((item) => item.id === userId)
...
app.get('/api/challenges', async (req, res) => {
  ...
  const progressByChallenge = user.challengeProgress || {}

  res.json({
    success: true,
    challenges: challengesData.challenges.map((challenge) => ({
      ...challenge,
      joined: Boolean(progressByChallenge[challenge.id]?.joined),
      completedSteps: Number(
        progressByChallenge[challenge.id]?.completedSteps || 0
      )
    }))
  })
})
...
app.post('/api/challenges/:challengeId/join', async (req, res) => {
  ...
  user.challengeProgress[challengeId] = {
    ...
    joined: true,
    completedSteps: Number(
      user.challengeProgress[challengeId]?.completedSteps || 0
    )
  }
...
app.post('/api/challenges/:challengeId/steps/complete', async (req, res) => {
  const userId = Number(req.body.userId)
  const challengeId = Number(req.params.challengeId)
  const usersData = await getUsersData()
  const challengesData = await getChallengesData()
  ...
  user.challengeProgress = user.challengeProgress || {}
  const current = user.challengeProgress[challengeId]

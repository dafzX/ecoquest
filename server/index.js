...
app.post(
  ...
  async (req, res, next) => {
    try {
      ...
      if (
        user.completedMissionIds.includes(
          missionId
        )
      ) {
        return res.status(409).json({
          success: false,
          message:
            'Mission ini sudah pernah diselesaikan.'
        })
      }
      ...
      res.json({
        success: true,
        message:
          `Mission selesai! Kamu mendapat ${mission.xp} XP.`,
        user: publicUser(user)
      })
    } catch (error) {
      next(error)
    ...
    try {
      ...
      if (!user || !reward) {
        return res.status(404).json({
          success: false,
          message:
            'User atau reward tidak ditemukan.'
        })
      }
      ...
      if (
        user.redeemedRewardIds.includes(
          rewardId
        )
      ) {
        return res.status(409).json({
          success: false,
          message:
            'Reward ini sudah pernah diklaim.'
        })
      }
      ...
      if (user.xp < reward.cost) {
        return res.status(400).json({
          success: false,
          message:
            'XP kamu belum cukup untuk menukar reward ini.'
        })
      }
      ...
      res.json({
        success: true,
        message:
          `Reward ${reward.title} berhasil diklaim.`,
        reward,
        user: publicUser(user)
      })
    } catch (error) {
    ...
    try {
      ...
      if (!user || !content) {
        return res.status(400).json({
          success: false,
          message:
            'User dan isi postingan wajib diisi.'
        })
      }
    ...
    try {
      ...
      if (!userId || !post) {
        return res.status(404).json({
          success: false,
          message:
            'User atau postingan tidak ditemukan.'
        })
      }
    ...
    try {
      ...
      if (!user || !challenge) {
        return res.status(404).json({
          success: false,
          message:
            'User atau challenge tidak ditemukan.'
        })
      }
    ...
    try {
      ...
      if (!current?.joined) {
        return res.status(400).json({
          success: false,
          message:
            'Ikuti challenge terlebih dahulu.'
        })
      }

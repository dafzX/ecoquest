      ...
      <div class="relative h-[108px] overflow-hidden bg-[#DCFCE7]">
        ...
        <div class="absolute bottom-3.5 left-4">
          <div class="flex items-center gap-1.5">
            ...
            <span class="text-[8px] font-medium text-[#15803D]">Tantangan Komunitas</span>
          </div>
        </div>
      </div>
      ...
      <div class="p-3.5">
        <div class="flex items-start gap-3">
          <div class="min-w-0 flex-1">
            <h2 class="text-[12px] font-bold leading-4 text-[#17211B]">{{ featuredChallenge.title }}</h2>
            <p class="mt-1 text-[8px] leading-[13px] text-[#718078]">{{ featuredChallenge.description }}</p>
          </div>
        ...
        <div class="mt-4">
          <div class="mb-1.5 flex items-center justify-between">
            <span class="text-[8px] text-[#718078]">{{ featuredChallenge.participants }} peserta</span>
            <span class="text-[8px] font-bold text-[#22C55E]">{{ featuredChallenge.progress }}%</span>
          </div>
          <div class="h-[5px] w-full overflow-hidden rounded-full bg-[#E5EFE8]">
            <div class="h-full rounded-full bg-[#22C55E] transition-all" :style="{ width: `${featuredChallenge.progress}%` }"></div>
          </div>
        </div>
        ...
        <div class="mt-3.5 flex items-center justify-between">
          <div class="flex items-center gap-1.5">
            ...
            <span class="text-[8px] text-[#718078]">{{ featuredChallenge.daysLeft }} hari lagi</span>
          </div>
          <RouterLink :to="`/challenges/${featuredChallenge.id}`" class="flex h-7 items-center gap-1 rounded-lg bg-[#22C55E] px-3 text-[8px] font-semibold text-white transition hover:bg-[#15803D] active:scale-[0.98]">
            Lihat Challenge
    ...
    <section>
      ...
      <div class="space-y-2.5">
        ...
        >
          <div class="flex items-start gap-3">
            <!-- Icon -->
            <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl" :class="getChallengeStyle(challenge.category)">
              <component :is="getChallengeIcon(challenge.category)" class="h-5 w-5" />
            </div>
            ...
            <div class="min-w-0 flex-1">
              <div class="flex items-start justify-between gap-2">
                <div class="min-w-0">
                  <h3 class="text-[10px] font-bold leading-4 text-[#17211B]">{{ challenge.title }}</h3>
                  <p class="mt-0.5 text-[8px] text-[#718078]">{{ challenge.participants }} peserta</p>
                </div>
                <span class="shrink-0 rounded-full bg-[#E8F8ED] px-2 py-1 text-[7px] font-semibold text-[#15803D]">
                  {{ challenge.daysLeft }} hari
                </span>
              </div>
              <p class="mt-1.5 text-[8px] leading-[13px] text-[#718078]">{{ challenge.description }}</p>

              <!-- Progress -->
              <div class="mt-2.5">
                <div class="mb-1 flex items-center justify-between">
                  <span class="text-[7px] text-[#98A39C]">Progress</span>
                  <span class="text-[7px] font-semibold text-[#22C55E]">{{ challenge.progress }}%</span>
                </div>
                <div class="h-[4px] w-full overflow-hidden rounded-full bg-[#E5EFE8]">
                  <div class="h-full rounded-full bg-[#22C55E]" :style="{ width: `${challenge.progress}%` }"></div>
                </div>
              </div>
            </div>
          </div>

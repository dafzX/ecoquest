  ...
  <main class="hidden md:block">
    <div class="min-h-screen px-8 py-8">
      <div class="mx-auto max-w-3xl">
        ...
        <!-- Main Card -->
        <div class="overflow-hidden rounded-3xl border border-[#E2E8F0] bg-white p-8">
          
          <div class="flex items-center justify-between mb-6">
            <span class="rounded-full bg-[#F0FDF4] px-4 py-1.5 text-sm font-semibold text-[#15803D]">
          ...
          <div>
            ...
            <p class="mt-2 text-base text-[#64748B] leading-relaxed">
              {{ step.description }}
            </p>
          </div>

          <!-- Action Hint -->
          <div class="mt-8 rounded-2xl bg-[#F8FAFC] p-6 border border-[#E2E8F0]">
          ...
          >
            {{ stepNumber < totalSteps ? 'Selesaikan Langkah Ini' : 'Selesaikan Mission' }}
            <Check class="h-5 w-5" />
          </button>
...
const progress = computed(() => {
  ...
  return Math.round(((props.stepNumber - 1) / props.totalSteps) * 100)
})
</script>
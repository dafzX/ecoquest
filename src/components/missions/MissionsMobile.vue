      ...
      <div v-if="filteredMissions.length" class="space-y-2.5">
        ...
        >
          <div class="flex gap-3">
            ...
            >
              <component :is="getCategoryIcon(mission.category)" class="h-5 w-5" />
            </div>
            ...
            <div class="min-w-0 flex-1">
              ...
              <!-- Progress -->
              <div v-if="mission.progress !== undefined && !mission.completed" class="mt-2">
                <div class="flex items-center justify-between">
                  <span class="text-[7px] text-[#98A39C]">Progress</span>
                  <span class="text-[7px] font-semibold text-[#22C55E]">{{ mission.progress }}%</span>
                </div>
              ...
              <!-- Completed -->
              <div v-if="mission.completed" class="mt-2 flex items-center gap-1 text-[7px] font-medium text-[#22C55E]">
          ...
          <RouterLink
            ...
            :class="mission.completed ? 'bg-[#F1F5F2] text-[#718078]' : 'bg-[#22C55E] text-white'"
          >
            {{ mission.completed ? 'Lihat Detail' : mission.progress ? 'Lanjutkan' : 'Mulai Quest' }}
      ...
      <article class="overflow-hidden rounded-[12px] border border-[#DCEBE0] bg-white">
        ...
        <div class="p-3.5">
          ...
          <div class="mt-3 flex items-center justify-between">
            <div>
              <p class="text-[7px] text-[#98A39C]">Peserta</p>
              <p class="mt-0.5 text-[10px] font-bold text-[#17211B]">320 Explorer</p>
            </div>
            <div class="text-right">
              <p class="text-[7px] text-[#98A39C]">Progress</p>
              <p class="mt-0.5 text-[10px] font-bold text-[#22C55E]">68%</p>
            </div>
          </div>
          <div class="mt-2 h-1.5 overflow-hidden rounded-full bg-[#E5EFE8]">
            <div class="h-full w-[68%] rounded-full bg-[#22C55E]"></div>
          </div>
          <RouterLink to="/challenges" class="mt-3 flex h-7 items-center justify-center gap-1 rounded-lg bg-[#22C55E] text-[8px] font-semibold text-white transition active:scale-[0.98]">
            Lihat Challenge
            <ArrowRight class="h-3 w-3" />
          </RouterLink>
        </div>
...
<script setup>
import {
  Search, ArrowRight, CheckCircle2, Trophy
